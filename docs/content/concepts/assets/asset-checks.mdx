...
Checks are supported per-asset, not per-partition. To write a check on a partitioned asset, you'll want to summarize the status of all the partitions in to one pass/fail for the asset.

See [this GitHub issue](https://github.com/dagster-io/dagster/issues/17005) for updates on per-partition checks.
