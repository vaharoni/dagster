...
### Checks on partitioned assets

To write a check on a partitioned asset, summarize the status of all the partitions as a single pass/fail for the asset.

Here's an example that checks that no partitions are null.

```python file=/concepts/assets/asset_checks/partitions.py
...
This is slightly different with some dataframe based io managers, which combine multiple partitions in to one dataframe. Here's an example that uses the <PyObject module="dagster_duckdb_pandas" object="duckdb_pandas_io_manager"/>.

```python file=/concepts/assets/asset_checks/db_partitions.py
import os
...
Checks are currently supported per-asset, not per-partition. See [this GitHub issue](https://github.com/dagster-io/dagster/issues/17005) for updates on per-partition checks.
